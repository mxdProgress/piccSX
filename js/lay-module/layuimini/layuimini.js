...
layui.define(["element", "jquery"], function(exports) {
    ...
    layuimini = new function() {
        ...
        this.config = function(name) {
            ...
            var config = {
                urlHashLocation: false, // 是否开启URL地址hash定位，默认开启。关闭后，刷新页面后将定位不到当前页，只显示主页
        ...
        this.initMenu = function(data) {
            ...
            $.each(data, function(key, val) {
                ...
                $.each(menuList, function(index, menu) {
                    ...
                    if (menu.children != undefined && menu.children != [] && menu.children.length != 0) {
                        ...
                        var buildChildHtml = function(html, child, menuParameId) {
                            ...
                            $.each(child, function(childIndex, childMenu) {
                                html += '<dd>\n';
                                if (childMenu.children != undefined && childMenu.children != [] && childMenu.children.length != 0) {
        ...
        /**
         * 根据title删除指定Tab
         * @param titDel  要删除的tab标题
         * 
         */

        this.getTitleDelTab = function(titDel) {
            var layuiminiTabInfo = JSON.parse(sessionStorage.getItem("layuiminiTabInfo"));
            for (obj in layuiminiTabInfo) {
                var title = layuiminiTabInfo[obj].title;
                var href = layuiminiTabInfo[obj].href;
                var titIndex = title.indexOf(titDel);
                var spanIndex = title.indexOf('</span>');
                var TitletextDel = title.slice(titIndex, spanIndex);
                if (TitletextDel == titDel) {
                    layuimini.delTabIfram(href);
                }
        ...
        /**
         *  @param titChange 要切换的tab标题
         */

        this.getChangeTab = function(titChange) {
            var layuiminiTabInfo = JSON.parse(sessionStorage.getItem("layuiminiTabInfo"));
            for (obj in layuiminiTabInfo) {
                var title = layuiminiTabInfo[obj].title;
                var href = layuiminiTabInfo[obj].href;
                var titIndexs = title.indexOf(titChange);
                var spanIndex = title.indexOf('</span>');
                var TitletextChange = title.slice(titIndexs, spanIndex);

                if (TitletextChange == titChange) {
        ...
        this.getTitleChangeTab = function(tit) {
            var layuiminiTabInfo = JSON.parse(sessionStorage.getItem("layuiminiTabInfo"));
            for (obj in layuiminiTabInfo) {
                var title = layuiminiTabInfo[obj].title;
                var href = layuiminiTabInfo[obj].href;
                var titIndex = title.indexOf(tit);
                var spanIndex = title.indexOf('</span>');
                var text = title.slice(titIndex, spanIndex);
                if (text == tit) {
                    parent.layui.element.tabChange('layuiminiTab', href);
                }
            }
         ...
         * @param tabId
         */
        this.delTabIfram = function(tabId) {
            var layuiminiTabInfo = JSON.parse(sessionStorage.getItem("layuiminiTabInfo"));
            if (layuiminiTabInfo != null) {
                delete layuiminiTabInfo[tabId];
                sessionStorage.setItem("layuiminiTabInfo", JSON.stringify(layuiminiTabInfo))
